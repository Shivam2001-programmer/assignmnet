    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/v1/get-insight"
        );
  ...
  const handleToggleFavorite = async (id) => {
    try {
      const response = await axios.put(
        `http://localhost:5000/api/v1/${id}/favorite`
      );
      setData((prevData) =>
        prevData.map((item) =>
          item.id === id
    ...
    <table className="min-w-full border-collapse border border-gray-300">
      <thead>
        <tr className="bg-gray-100">
          <th className="border border-gray-300 p-2 text-left">Domain Name</th>
          ...
          <th className="border border-gray-300 p-2 text-left">Media Links</th>
